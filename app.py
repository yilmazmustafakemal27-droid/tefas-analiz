"""
TEFAS Fon Analiz Aracı — Web Uygulaması (v9)
=============================================
v9 GÜNCELLEMELERİ:
- Backtest 'hata' mesajı kullanıcıya görünür şekilde anlatılıyor
- Eksik açıklamalar bağlandı: SMA, yıllık eğim, rolling beta/alpha/IR, rejim_stat
- Backtest minimum veri 250 işlem gününe düşürüldü (önceden ~400 takvim günüydü)
"""
...
with st.sidebar:
    ...
    gun_sayisi = st.select_slider(
        ...
        value=1000,
        help="Daha uzun veri = daha sağlam istatistik. Backtest için 750+ gün önerilir."
    )

    egt_g = int(gun_sayisi * 2/3)
    tah_g = int(gun_sayisi * 1/3)
    st.caption(f"📊 Backtest: **{egt_g}g** eğitim → **{tah_g}g** tahmin (yaklaşık)")
...
if not analiz_basla:
    ...
    with col_b:
        st.markdown("""**2. Parametreleri ayarlayın**\n\nBeklediğiniz enflasyon ve veri uzunluğunu seçin.""")
    with col_c:
    ...
    with tab1:
        ...
        fig_fiyat.add_trace(go.Scatter(
            x=df['date'], y=df['price'], mode='lines', name='Fiyat',
        ...
        if len(df) >= 252:
            ...
            fig_fiyat.add_trace(go.Scatter(
                x=son_252['date'], y=trend, mode='lines', name='252g log-trend',
        ...
        with col_l:
            ...
            # Z-skor + konum yorumu (her ikisi de açıklamalı)
        ...
        # YENİ: Rejim istatistikleri başlığı + açıklama
    ...
    with tab2:
        ...
        st.caption("Skorlar 0-100 arası bilgilendiricidir. Yüksek skor = göstergeler olumlu, düşük skor = göstergeler olumsuz.")
        ...
        fig_sinyal.update_layout(
            height=300, margin=dict(l=10, r=40, t=10, b=30),
        ...
        # Rolling beta/alpha — YENİ: ÜSTÜNE AÇIKLAMA POPOVER'LARI EKLENDİ
        if sonuc['rolling_ba'] is not None:
            st.markdown("---")

            col_rt1, col_rt2 = st.columns([20, 1])
            with col_rt1:
                st.markdown("### Zamana Göre Performans (Rolling Metrikler)")
            with col_rt2:
                with st.popover("❓", help="Rolling metrikler ne demek?"):
                    st.markdown("#### Rolling Metrikler")
                    st.markdown(
                        "**Rolling metrik**, bir göstergenin **tek bir genel değer** yerine "
                        "**zaman içinde nasıl değiştiğini** gösteren versiyonudur.\n\n"
                        "Burada 6 aylık (126 işlem günü) kayar pencere kullanılıyor. "
                        "Her noktada son 6 ayın verisi alınıp Beta/Alpha/IR hesaplanıyor.\n\n"
                        "💡 *Düz çizgi = tutarlı yönetim. Çok dalgalanan çizgi = stratejide değişimler var.*"
                    )

            # 3 ayrı popover — her grafiğin yanında
        ...
        if sonuc['rolling_sh'] is not None:
            ...
            with col_rsh1:
                st.markdown("#### Rolling Sharpe (6ay)")
            with col_rsh2:
            ...
            fig_rsh.add_trace(go.Scatter(x=rsh['date'], y=rsh['sharpe'],
                                         line=dict(color='#2E7D5C', width=1.5),
                                         showlegend=False))
            fig_rsh.add_hline(y=1.0, line_dash="dot", line_color="#888",
                            annotation_text="Sharpe = 1 (iyi eşik)")
    ...
    with tab4:
        ...
        st.caption(f"{senaryo_sayisi:,} senaryo. Bu bir tahmin değil, **olasılık dağılımıdır**.")
        ...
        fig_mc.add_trace(go.Scatter(x=gun_aralik, y=p5_yol, mode='lines',
            name='P5 (kötü senaryo)', line=dict(color='#B83A3A', width=1.5, dash='dash')))
        fig_mc.add_trace(go.Scatter(x=gun_aralik, y=p50_yol, mode='lines',
            name='P50 (medyan)', line=dict(color='#0F4C81', width=2.5)))
        fig_mc.add_trace(go.Scatter(x=gun_aralik, y=p95_yol, mode='lines',
            name='P95 (iyi senaryo)', line=dict(color='#2E7D5C', width=1.5, dash='dash')))
    ...
    # ----- TAB 5: BACKTEST (YENİ — HATA MESAJI GÖSTERİYOR) -----
    with tab5:
        ...
        # YENİ: Backtest çalışmadıysa NEDEN çalışmadığını söyle
        if bt is None:
            if bt_hata:
                st.error(f"❌ **Backtest çalıştırılamadı:**\n\n{bt_hata}")
                aciklama_expander("backtest_hata", bt_hata)
            elif not backtest_calistir:
                st.info("ℹ️ Backtest sidebar'dan kapatılmış. Çalıştırmak için 'Walk-forward backtest çalıştır' kutusunu işaretle.")
            else:
                st.warning("⚠️ Backtest sonuç üretmedi. Geçmiş veriyi (sidebar) artırmayı dene.")
        else:
            st.success(
                f"✅ Backtest tamamlandı: **{bt['test_sayisi']} test noktası** "
                f"(denenen: {bt.get('denenen_test', '?')}, atlanan: {bt.get('hata_sayisi', 0)})"
            )

            st.markdown(f"""
            **Test Yapılandırması:**
            - Eğitim verisi: **{bt['egitim_gun']} işlem günü** (geçmiş verinin 2/3'ü)
            - Tahmin ufku: **{bt['tahmin_ufku_is']} işlem günü** (≈ {bt['tahmin_ufku_takvim']} takvim günü)
            - Tamamlanan test sayısı: **{bt['test_sayisi']}**
            """)
            ...
            if bs is not None:
                ...
                with col_st1:
                    ...
                    st.markdown(
                        ...
                        f"border-radius: 8px; border: 1px solid #E8E5DD;'>"
                        f"<div style='font-size: 0.85rem; color: #888; text-transform: uppercase; "
                        f"letter-spacing: 0.1em; margin-bottom: 8px;'>Standart Skor</div>"
                ...
                with col_st2:
                    ...
                    st.markdown(f"""
                    Farklı tahmin ufukları doğrudan karşılaştırılamaz. **Standart skor**, sapma
                    büyüklüğünü **√t kuralıyla yıllık eşdeğere çevirir** ve karşılaştırma imkânı verir.

                    **Mevcut ayar:** {bt['tahmin_ufku_is']} işlem günü tahmin ufku
                    """)
            ...
            with col_h1:
                aciklamali_metrik("Bant Kapsama", f"%{bs['ham_bant']:.0f}" if bs else "N/A",
                                 "backtest_bant", bs['ham_bant'] if bs else None,
                                 delta="Hedef: %90")
            with col_h2:
                aciklamali_metrik("Yön Doğruluğu", f"%{bs['ham_yon']:.0f}" if bs else "N/A",
                                 "backtest_yon", bs['ham_yon'] if bs else None,
                                 delta="Random: %50")
            with col_h3:
                if bs:
            ...
            fig_bt = make_subplots(rows=1, cols=2,
                subplot_titles=("P50 Tahmin vs Gerçekleşen", "Zaman İçinde Sapma (%)"),
                column_widths=[0.55, 0.45])

            renkler_scatter = ['#2E7D5C' if x else '#B83A3A' for x in df_bt['bant_ici']]
            fig_bt.add_trace(go.Scatter(
                x=df_bt['p50'], y=df_bt['reel_gercek'], mode='markers',
            ...
            fig_bt.add_trace(go.Bar(
                x=df_bt['test_tarihi'], y=df_bt['sapma_yuzde'],
            ...
            st.caption("🟢 Yeşil: gerçek değer P5-P95 bandı içinde. 🔴 Kırmızı: bandın dışında.")
    ...
    with tab6:
        ...
        if sonuc['backtest_skor'] is not None:
            ...
            detay['Backtest'] = {
                ...
                'Bant kapsama (ham)': f"%{bs['ham_bant']:.1f}",
                'Yön doğruluğu (ham)': f"%{bs['ham_yon']:.1f}",
            ...
            detay['Backtest'] = {'Durum': f"Çalıştırılamadı: {sonuc['backtest_hata'][:100]}..."}
        ...
        st.download_button("📥 Fon fiyat geçmişi (CSV)", data=csv_data,
...
Bu uygulama TEFAS'ta listelenen fonların kamuya açık fiyat verilerini istatistiksel
olarak analiz eden bir <strong>eğitim ve araştırma aracıdır</strong>. Yatırım tavsiyesi,
danışmanlığı veya öneri içermez. Yatırım kararı vermeden önce <strong>SPK lisanslı bir
yatırım danışmanına</strong> başvurun.
</div>
""", unsafe_allow_html=True)
